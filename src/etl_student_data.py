...
#  Cargo los datos de Mirza
df = pl.read_csv("data/raw/mobile_phone_screen_time_dataset.csv")

#  Filtro estricto para estudiantes (nuestra población de estudio)
df_estudiantes = df.filter(pl.col("Occupation") == "Student")

#  Selección inteligente de variables (Mantenemos País y Continente para análisis transcultural)
columnas_proyecto = [
...
#  Cálculo del IFA (Neuro-métrica de fragmentación)
df_clean = df_clean.with_columns([
    (pl.col("Daily_Screen_Time_Hours") * (pl.col("Screen_Unlocks_Per_Day") + 1).log()).alias("IFA")
])

#  Segmentación Cualitativa de la Frecuencia de Desbloqueo (Comportamiento Estratégico)
df_clean = df_clean.with_columns([
